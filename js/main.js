...
if (stellar.plugins.swiper) {
  ...
  if (swiper_container != undefined) {
    ...
    stellar.loadScript(stellar.plugins.customSwiperTopArticle.js, {defer:false});
    stellar.loadScript(stellar.plugins.customSwiperTopArticle.init_js, {defer:true});
  }
  // 轮播插件
...
// 按需加载fontawesome图标css
if (stellar.plugins.fontawesome) {
  const fontawesome_i = document.getElementsByTagName("i");
  if (fontawesome_i != undefined && fontawesome_i.length > 0) {
      stellar.loadCSS(stellar.plugins.fontawesome.animation_css);
      stellar.loadCSS(stellar.plugins.fontawesome.css);
  }
}

// 按需加载获取诗词语句js
if (stellar.plugins.poemGet) {
  const poem = document.getElementById('poem');
  if (poem != undefined) {
    stellar.jQuery( () => {
      stellar.loadScript(stellar.plugins.poemGet.js, {defer:true})
    })
  }
}

// 按需加载网页底部网站运行时间js和卜算子统计js
if (stellar.plugins.siteRunDateAndBusuanzi) {
  // 网页底部网站运行时间
  const timeDate = document.getElementById('timeDate');
  if (timeDate != undefined) {
    stellar.loadScript(stellar.plugins.siteRunDateAndBusuanzi.time_js, {defer:true});
  }
  // 卜算子统计
  const busuanzi_value_site_uv = document.getElementById('busuanzi_value_site_uv');
  if (busuanzi_value_site_uv != undefined) {
    stellar.loadScript(stellar.plugins.siteRunDateAndBusuanzi.busuanzi_js, {defer:true});
  }
}

// 人生倒计时js
if (stellar.plugins.timelife) {
  const timelife = document.getElementById('timelife');
  if (timelife != undefined) {
    stellar.jQuery( () => {
      stellar.loadScript(stellar.plugins.timelife.js, {defer:true})
    })
  }
}
